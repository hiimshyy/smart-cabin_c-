...
- [x] 1. Extract Benchmark_Module (lowest risk — pure accounting + printf)
  - [x] 1.1 Create `src/benchmark.h` and `src/benchmark.cpp`
...
- [x] 2. Extract Config_Module (CLI parsing + usage text)
  - [x] 2.1 Create `src/app_config.h` with `AppConfig` and interface
  ...
  - [x] 2.2 Implement `parse_args` and `print_usage` in `src/app_config.cpp`
...
- [x] 3. Checkpoint - review Benchmark + Config extraction
  - Ensure all property tests pass, ask the user if questions arise.

- [x] 4. Extract Video_IO_Module (capture + display threads + gst helpers)
  - [x] 4.1 Create `src/video_io.h` with slots and function declarations
  ...
  - [x] 4.2 Implement the six functions verbatim in `src/video_io.cpp`
...
- [x] 5. Extract face_label.h and Overlay_Module
  - [x] 5.1 Create shared data-only `src/face_label.h`
  ...
  - [x] 5.2 Create `src/overlay.h` with `UiScale` and draw function declarations
    ...
    - _Requirements: 4.2_
  - [x] 5.3 Implement `UiScale::compute` and draw functions verbatim in `src/overlay.cpp`
...
- [x] 6. Checkpoint - review Video_IO + Overlay extraction
  - Ensure all property tests pass, ask the user if questions arise.

- [x] 7. Trim main.cpp to model init + pipeline orchestration + shutdown
  - [x] 7.1 Wire main.cpp to the extracted modules and remove moved definitions
...
- [x] 8. Update Makefile for the new app-only modules
  - [x] 8.1 Add `APP_ONLY_SRCS` and append it to `APP_SRCS_CPP`
...
- [x] 9. Static-review checklist on the Windows dev host
...
- [x] 10. Linux build verification (VERIFIED on Orange Pi — env is Linux, not the Windows the spec assumed)
  - [x]* 10.1 Run `make all` on the Target_Toolchain
